...
SUBSET = 'grammar_correct_1fps_16maxframes'
OUTPUT_SUBSET = 'grammar_correct_1fps_16maxframes_1280xtokens_far_compress'
include_datasets = [
    # 'LLaVA-Hound',
    # 'LLaVA-Video-178K',
    'ShareGPT4Video'
...
def pack_single_row(item):
    ...
    image_text = ''
    seq_lens = dict()
    for i, (image_name, base64_image) in enumerate(base64_image_dict.items()):
        if i == len(base64_image_dict) - 1:
            max_pixels = MAX_PIXELS
        else:
            max_pixels = MAX_PIXELS / 4
...
if __name__ == '__main__':
    ...
    success_files = []
    failed_files = []
    for filename in tqdm(points):
        sub_filename = filename.split('/')[-1]
        print(f'Processing {filename}')
        cur_folder = os.path.join(
            root, filename, 'data', SUBSET)
        all_jsonl_files = find_all_files(cur_folder, 'jsonl')
        if len(all_jsonl_files) == 0:
            print(f'{cur_folder} does not have jsonl files')
            continue
        all_jsonl_files_cur_rank = dist_split_files(all_jsonl_files)
        for cur_file in tqdm(all_jsonl_files_cur_rank):
            data = read_jsonl_file(cur_file)
            if len(data) == 0:
                print(f'{cur_file} is empty')
                continue
            cur_file_name = os.path.basename(cur_file).split('.')[0]
            output_file = os.path.join(
                root, filename, f'jsonl/qwen2vl-{OUTPUT_SUBSET}/{cur_file_name}.jsonl')
