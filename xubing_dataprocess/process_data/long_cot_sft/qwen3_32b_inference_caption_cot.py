...
PROMPT = (
'Now I have a detailed description of an image.\n'
'This image could be anything, such as a nature photo, a table, a scientific image, etc.\n'
'Please answer a question about the image based on its description.\n'
...
def vllm_batch_inference(item):
    ...
    description = frame_metadata["conversations"][1]['qwen2.5vl-72b']
    messages = []
    content = []
    input_text = PROMPT.format(description=description, question=question)
    ...
    try:
        ...
        frame_metadata["qwen3_32b_caption_content"] = chat_response.choices[0].message.content
        frame_metadata["qwen3_32b_caption_reasoning_content"] = chat_response.choices[0].message.reasoning_content
        frame_metadata["qwen3_32b_caption_cot"] = f"<think>{chat_response.choices[0].message.reasoning_content}</think> {chat_response.choices[0].message.content}"
        ...
        try:
            ...
            frame_metadata["qwen3_32b_caption_reasoning_content"] = chat_response.choices[0].message.reasoning_content
            frame_metadata["qwen3_32b_caption_cot"] = f"<think>{chat_response.choices[0].message.reasoning_content}</think> {chat_response.choices[0].message.content}"
            frame_metadata["qwen3_32b_truncation"] = 1
            return frame_metadata
        except Exception as e:
            print(f"Error occurred again while processing: {e}")
            frame_metadata["qwen3_32b_caption_content"] = ""
            frame_metadata["qwen3_32b_caption_reasoning_content"] = ""
            frame_metadata["qwen3_32b_caption_cot"] = ""
            return frame_metadata
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        dataset += '_vlm_infer_Qwen72B_caption'
        output_dataset = dataset + '_qwen3_cot'
