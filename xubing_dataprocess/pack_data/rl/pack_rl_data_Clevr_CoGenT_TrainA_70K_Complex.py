...
# pip install pyarrow
parquets_path = "/mnt/cephfs/xubingye/wfs/datasets/rl-category/Clevr_CoGenT_TrainA_70K_Complex/data"
use_val = True
...
for i, df in enumerate(parquet_files):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        new_row = {
            ...
            'data_source': "MMInstruction/Clevr_CoGenT_TrainA_70K_Complex",
            'prompt': ensure_list({'content': image_placeholder + row['problem'] + format_prompt, 'role': 'user'}),
            'ability': "Counting",
            'reward_model': {'ground_truth': answer, 'style': 'rule'},
            'extra_info': {'thinking': "", 'problem': row['problem'], 'solution': row['solution']}
        }
