...
# pip install pyarrow
parquets_path = "/mnt/cephfs/xubingye/wfs/datasets/rl-category/zerobench/data"
use_val = False
...
for i, df in enumerate(parquet_files):
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        assert len(images) == len(row['question_images'])
        for idx, image in enumerate(images):
            images[idx]['path'] = row['question_images'][idx]
        new_row = {
            'images': ensure_list(row['question_images_decoded']),
            'data_source': "hiyouga/geometry3k",
            'prompt': ensure_list({'content': image_placeholder * len(images) + row['question_text'] + format_prompt, 'role': 'user'}),
            'ability': "puzzle",
            'reward_model': {'ground_truth': row['question_answer'].strip(), 'style': 'rule'},
            'extra_info': {'thinking': row.get('pattern', ''), 'problem': row.get('question_text', ''), 'solution': row.get('question_answer', '').strip()}
        }
        lst.append(new_row)
        if len(lst) > 20:
