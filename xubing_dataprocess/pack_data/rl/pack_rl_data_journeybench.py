...
# pip install pyarrow
parquets_path = "/mnt/cephfs/xubingye/wfs/datasets/rl-category/journeybench-multi-image-vqa/data"
use_val = False
...
for i, df in enumerate(parquet_files):
    ...
    for index, row in tqdm(df.iterrows()):
        # breakpoint()
        new_row = {
            'images': ensure_list(row['images']),
            'data_source': "hiyouga/geometry3k",
            'prompt': ensure_list({'content': row['problem'] + format_prompt, 'role': 'user'}),
            'ability': "puzzle",
            'reward_model': {'ground_truth': row['answer'], 'style': 'rule'},
            'extra_info': {'thinking': row.get('pattern', ''), 'problem': row['problem'], 'solution': row['answer']}
        }
