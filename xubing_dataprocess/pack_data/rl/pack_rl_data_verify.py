...
# pip install pyarrow
parquets_path = "/mnt/cephfs/xubingye/wfs/datasets/rl-category/verify-teaser/data"
use_val = False
...
for i, df in enumerate(parquet_files):
    ...
    for index, row in tqdm(df.iterrows()):
        answer = row['answer']
        assert answer in row['options']
        answer = index_to_letter(np.where(row['options'] == answer)[0][0])
        problem = image_placeholder + row['question']
        for idx, option in enumerate(row['options']):
            if option in ['a', 'b', 'c', 'd']:
                option = option.upper()
            problem += f" {index_to_letter(idx)}: {option}."
        new_row = {
            'images': ensure_list(row['image']),
            'data_source': "hiyouga/geometry3k",
            'prompt': ensure_list({'content': problem + format_prompt, 'role': 'user'}),
            'ability': "Math",
            'reward_model': {'ground_truth': answer.strip(), 'style': 'rule'},
            'extra_info': {'thinking': row.get('reasoning', ''), 'problem': row.get('question', ''), 'solution': answer.strip()}
        }
