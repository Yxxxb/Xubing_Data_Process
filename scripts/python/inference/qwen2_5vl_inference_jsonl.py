...
if __name__ == "__main__":
    ...
    print(f"args.partition {args.partition}...")
    print(f"deal with {all_jsonls} files...")

    for _jsonl, _path in zip(all_jsonls, all_output_paths):
        data = read_jsonl_file(_jsonl)
        data = [[item, args.model_name, args.model_path] for item in data]
        results = multi_process_with_append(
            query_single_item_wrapper,
            data,
            min(args.num_workers, len(data)),
        )

        output_jsonl = os.path.join(_path, f"{os.path.basename(_jsonl)}")
        if len(results) > 0:
